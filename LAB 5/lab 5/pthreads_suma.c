...
int main( int argc, char *argv[] ){
  ...
  for(i=0; i<LICZBA_W; i++ ) 
    pthread_create( &watki[i], NULL, calka, (void *) &indeksy[i] );
...
void *calka( void *arg_wsk){
  ...
  double dx = (3.14159265359-0)/N2;
  moj_id = *( (int *) arg_wsk ); 
  double x2=3.14159265359;
  double x1 = 0; 
  double c=0.0; 
  j=N2/LICZBA_W;

  if(moj_id == (LICZBA_W-1))
	rekompensata = N2%LICZBA_W;

  for( i=j*moj_id+1; i<=j*(moj_id+1+rekompensata); i++){ 
	x2=x1+dx;  
	c+=0.5*(f(x1)+f(x2))*dx; 
	x1=x2;
  }
